...
// import coins
var coins_low='img/coins_low.jpg'

var coins_high='img/coins_high.jpg'
...
var red_cross='img/red_cross.jpg'
...
// file_index_p1=_.pullAll(file_index_p1,[file_index_p1[4]]);
...
var trial ={
    ...
    if (sequence==1){
      if (data.key_press==jsPsych.pluginAPI.convertKeyCharacterToKeyCode('w')&&(shuffleda[trials-1]<=5||shuffleda[trials-1]>15)){
                ...
                return "<img src='"+ red_cross +"'>" 
            }
        }else if(sequence==2){
       if (data.key_press==jsPsych.pluginAPI.convertKeyCharacterToKeyCode('w')&&(shuffleda[trials]<=10&&shuffleda[trials]>5)|(shuffleda[trials]>10&&shuffleda[trials]<=15)){
                left=left+1;
                jsPsych.setProgressBar(left*0.1);
                return "<img src='"+ circle +"'>"}
...
var ins_2_key_map_a=file_index_p2[6];
}
else{
...
if (group==1){
    var key_outcome_h=file_index_p2[2];
    var key_outcome_l=file_index_p2[1];
}else{
    var key_outcome_h=file_index_p2[3];
    var key_outcome_l=file_index_p2[10];
}
...
var hello_trial_2 = {
    ...
    pages: [   '<img src='+ file_index_p2[0] +'></img>',
                ...
                '<img src='+ file_index_p1[9] +'></img>',
                '<img src='+ key_outcome_h +'></img>',
                '<img src='+ key_outcome_l +'></img>',
...
outcome_money = {
        ...
        stimulus:function(){
            if (right>=10 && group ==2){
                return "<img src='"+ coins_low +"'>" 
            }else if (left>=10 && group==2){
                return "<img src='"+ coins_high +"'>" 
            }else if (right>=10 && group ==1){
                return "<img src='"+ coins_high +"'>" 
            }else if (left>=10&& group==1){
                return "<img src='"+ coins_low +"'>" 
            }else{
                return "<p>You did not get the coins</p>"    
            }
        },
        trial_duration:1000 
}
    nothing={
      ...
      var test_procedure2={
        timeline: [fixation,trial_p2_p,outcome_money,nothing],
        repetitions:2
        }
...
var trial_p2_a ={
 ...
 stimuli ={
     ...
     choices: ["w","o"],
     stimulus: function(){
         if (shuffleda_2[trials_2-1]<=10){
             return "<img src='"+ yellow_f +"'>" 
         }else if(shuffleda_2[trials_2-1]>10){
             return "<img src='"+ blue_f +"'>" 
         }
     }
 },
 ...
 outcome = {
     ...
     stimulus: function (){ 
         var data = jsPsych.data.get().last(1).values()[0];
 if (sequence==1){
   if (data.key_press==jsPsych.pluginAPI.convertKeyCharacterToKeyCode('w')&&shuffleda_2[trials_2-1]<=10){
             left=left+1;
             jsPsych.setProgressBar(left*0.1);
             return "<img src='"+ circle +"'>"}
         else if (data.key_press==jsPsych.pluginAPI.convertKeyCharacterToKeyCode('o')&&shuffleda_2[trials_2-1]>10){
             right=right+1;
             jsPsych.setProgressBar(right*0.1);
             return "<img src='"+ circle +"'>"
         }else {
    ...
    if (data.key_press==jsPsych.pluginAPI.convertKeyCharacterToKeyCode('o')&&shuffleda_2[trials_2-1]<=10){
             right=right+1;
             jsPsych.setProgressBar(right*0.1);
             return "<img src='"+ circle +"'>"}
         else if (data.key_press==jsPsych.pluginAPI.convertKeyCharacterToKeyCode('w')&&shuffleda_2[trials_2-1]>10){
             left=left+1;
             jsPsych.setProgressBar(left*0.1);
             return "<img src='"+ circle +"'>"
         }else {
 ...
 repetitions:1000
   ...
   outcome_money = {
    ...
    stimulus:function(){
        ...
        right=0;
        left=0;
        if (right1>=10 && group ==2){
            num_coins_low=num_coins_low+1;
            return "<img src='"+ coins_low +"'>" 
        }else if (left1>=10 && group==2){
            num_coins_high=num_coins_high+1;
            return "<img src='"+ coins_high +"'>" 
        }else if (right1>=10 && group ==1){
            num_coins_high=num_coins_high+1;
            return "<img src='"+ coins_high +"'>" 
        }else if (left1>=10&& group==1){
            num_coins_low=num_coins_low+1;
            return "<img src='"+ coins_low +"'>" 
        }else{
            return "<p>You did not get the coins</p>"    
    ...
    trial_duration:1000,
    on_finish:function(){
        jsPsych.data.addProperties({num_coins_l:num_coins_low,num_coins_h:num_coins_high});
    }
   ...
   var test_procedure2_a={
     timeline: [fixation,trial_p2_a,outcome_money, nothing],
     repetitions:2
     }
...
summary_2={
    ...
    stimulus:function(){
        var data = jsPsych.data.get().last(1).values()[0];
        num_coins_l=data.num_coins_l
        num_coins_h=data.num_coins_h
        return '<p>You have collected '+  num_coins_l+' 5 cents coins and '+ num_coins_h+ ' 20 cents coins.</p>'+
                '<br>'+
...
if (order==1){
    ...
    var reward1=file_index_p3[2];
    var reward2=file_index_p3[4];
}else{
    var ins_3_map_p=file_index_p3[7];
    var ins_3_map_a=file_index_p3[9];
    var reward1=file_index_p3[4];
    var reward2=file_index_p3[2];
}
var hello_trial_3 = {
    type: 'instructions',
    pages: [   '<img src='+ file_index_p3[0] +'></img>',
                '<img src='+ file_index_p1[6] +'></img>', 
                '<img src='+ file_index_p3[1] +'></img>', 
                '<img src='+ reward1 +'></img>',
                '<img src='+ file_index_p3[3] +'></img>',
                '<img src='+ reward2 +'></img>',
...
var phase_3 ={
    ...
    timeline_variables: [
        { cue: s1, outcome: coins_low },
        { cue: s2, outcome: coins_high }
        ],
...
summary_3={
    ...
    choices: ['space'],
    stimulus:function(){
        sum=(num_coins_l+2)*5+(num_coins_h+2)*20; 
...
var trial ={
    ...
    if (sequence==1){
      if (data.key_press==jsPsych.pluginAPI.convertKeyCharacterToKeyCode('w')&&(shuffleda_4[trials_4-1]<=10||shuffleda_4[trials_4-1]>30)){
                ...
                jsPsych.setProgressBar(left*0.1);
                return "<img src='"+ circle +"'> "}
            else if (data.key_press==jsPsych.pluginAPI.convertKeyCharacterToKeyCode('o')&&(shuffleda_4[trials_4-1]<=30&&shuffleda_4[trials_4-1]>10)){
                right=right+1;
                jsPsych.setProgressBar(right*0.1);
                return "<img src='"+ circle +"'> "
            }else {
       ...
       if (data.key_press==jsPsych.pluginAPI.convertKeyCharacterToKeyCode('w')&&(shuffleda_4[trials_4-1]<=30&&shuffleda_4[trials_4-1]>10)){
                left=left+1;
                jsPsych.setProgressBar(left*0.1);
                return "<img src='"+ circle +"'> "}
...
// preload stimuli
file_index_p1.push(s1,s2,coins_low,coins_high,gray_square,blue_moon,blue_star,yellow_moon,yellow_star,blue_f,yellow_f,red_cross,circle)
...
jsPsych.init({
    timeline: timeline,
    preload_images:[file_index_p1,file_index_p2,file_index_p3,file_index_p4],
    exclusions: {
        min_width: 800,
        min_height: 600
      },
